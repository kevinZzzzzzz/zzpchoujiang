...
isParam = {
    ...
    fjMap: {},
    dhMap: {
        3712676: [1188, "Kar98K-星神"],
        3712677: [958, "幻神-星耀 皮肤"],
        3712678: [758, "Kar98K-星神音效卡（不可交易）"],
        3712679: [958, "星际战将"],
        3712680: [758, "毁灭-星耀 皮肤"],
        3712681: [558, "屠龙-星耀 皮肤"],
        3712687: [278, "AN94-超新星"],
        3712688: [248, "AK12-天启"],
        3712689: [198, "炼狱-星空"],
        3712690: [108, "羊驼玩偶"],
        3712692: [15, "交易专用钥匙x5"],
    ...
    xydMap: {
        3712780: [10, "游戏内王者夺宝红宝石x1"],
        3712791: [10, "游戏内王者轮回蓝宝石x1"],
        3712792: [10, "游戏内王者许愿黄宝石x1"],
        3712793: [10, "本活动抽奖钥匙x1"],
    },
    cddhMap: {
        3712762: [108, "星际战将"],
...
milo.ready(function () {
    ...
    window.confirm = function (msg, callback, ishow = false, cancelCallback) {
        ...
        // 取消按钮事件
        $("#poptips4 .popbtn2").attr(
            "href",
            "javascript:window.confirm_cancel_call();"
        );
    ...
    $(".popxanzbx a").click(function () {
        ...
        $("#popzix  .popget1").attr(
            "href",
            "javascript:ACT.getCdBoxGift(" + _idx + ",1);"
        );
        $("#popzix  .popget5").attr(
            "href",
            "javascript:ACT.getCdBoxGift(" + _idx + ",5);"
        );
    });
});
...
var Hx = {
    ...
    getAreaName: function (area, call) {
        var url =
            location.protocol +
            "//gameact.qq.com/comm-htdocs/js/game_area/cf_server_select.js";
...
var ACT = {
                ...
                var flow_1017401 = {
                    ...
                    success: function (res) {
                        ...
                        if (hasPay > 0) {
                            if (parseInt(jData.holdarr.fankui.iUsedNum) == 0) {
                                $(".btnFeedBack")
                                    ...
                                    .attr(
                                        "href",
                                        "javascript:TGDialogS('popfank')"
                                    );
                            } else {
                                $(".btnFeedBack")
                                    .addClass("gray")
                                    .attr("href", "javascript:;"); // 已填写
                            }
                        }
                        ...
                        if (parseInt(jData.holdarr.fankuilq.iUsedNum) == 0) {
                            $(".btn_getFkGfit")
                                .removeClass("gray")
                                .attr("href", "javascript:ACT.amsSayLq()");
                        } else {
                            $(".btn_getFkGfit")
                                .addClass("gray")
                                .attr("href", "javascript:;")
    ...
    amsZanQu(rid, name) {
        ...
        var msg =
            ...
            "】吗？唯一性道具在同一大区内，游戏仓库无法重复到账，请谨慎选择";
    ...
    hasPropGift: function (item) {
        ...
        var msg =
            "您确定领取星耀值×5" + "到 【" + loginInfo.qqGameArea + "】 吗？";
    ...
    wnExchangeGift: function (item) {
        ...
        confirm(msg, function () {
            request(exchangeApi, "post", {
                eventId,
                consumableNum: _config[0],
                eventItemName: _config[1],
                consumableName: "2025万能碎片",
            }).then((res) => {
                if (res.code == 200) {
                    getInfo();
    ...
    cdExchangeGift: function (item) {
        ...
        var msg =
            "您确定消耗" + _config[0] + "星耀值兑换【" + _config[1] + "】吗？";
...
function isDJApp() {
    var result =
        milo.cookie.get("djc_appVersion") != null ||
        typeof HostApp != "undefined" ||
        milo.request("djc_appVersion");
    return result;
}
...
function getInfo() {
    ...
    getClaim("星神5回馈按钮", () => {
        $(".p4bxbtnget").eq(0).addClass("gray");
    });
    getClaim("超新星回馈按钮", () => {
        $(".p4bxbtnget").eq(1).addClass("gray");
    });
    getClaim("天启回馈按钮", () => {
        $(".p4bxbtnget").eq(2).addClass("gray");
    });
