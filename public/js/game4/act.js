...
milo.ready(function () {
    ...
    window.confirm = function (msg, callback) {
        ...
        $("#confirm .btn-recharge3").attr("href", "javascript: window.confirm_call();");
        TGDialogS('confirm');
    }
    ...
    window.confirm_dt = function (msg, callback, ishow = 5) {
        ...
        Milo.aParams.interval = setInterval(() => {
            ...
            if (ishow === 0) {
                ...
                clearInterval(Milo.aParams.interval);
                $("#confirm_dt .btn-recharge3").attr("href", "javascript: window.confirm_dt_call();");
            }
        }, 1000);
...
var Hx = {
            ...
            if (res.code == 200) {
                ...
                if (arr.length == 1) {
                    ...
                    $.each(arr, function (k, v) {
                        $("#jlname").html(`<p style="letter-spacing: 1px">${v.name}${v.quantity > 1 ? 'x' + v.quantity : ''}</p>`);
                    })
                    TGDialogS("popdc");
                } else {
                    $("#choulist").html("");
                    $.each(arr, function (k, v) {
                        $("#choulist").append('<p style="letter-spacing: 1px">' + v.name + `${v.quantity > 1 ? 'x' + v.quantity : ''}` + '</p>');
                    })
                    TGDialogS("popJl");
                }
                getInfo()
    ...
    gift(page) {
        ...
        return false
        if (page == "0") return;
        request(getPackListApi, 'get', { page, limit: 8 }, page != 1).then(res => {
            if (res.code == 200) {
                ...
                $("#showMyGiftContent1 .now_page").html(pageNow + "/" + pageTotal);
                $("#showMyGiftContent1 .btn_prev").attr("href", "javascript:Hx.gift(" + pre + ");");
                $("#showMyGiftContent1 .btn_next").attr("href", "javascript:Hx.gift(" + next + ");");
    ...
    zcx(page) {
        ...
        request(`${getTankListsApi}`, 'post', {
            ...
            if (res.code == 200) {
                    ...
                    temporaryBox.forEach(item => {
                        if (item.status == 0) {
                            ...
                            <tr>
                                <td>${item.name}</td>
                                <td>
                                    <a href="javascript:Hx.zcxlq(${item.id}, '${item.name}')" style="background-color:red;color:#fff;">[发送]</a>
                                </td>
                                <td>
                                    <a href="javascript:Hx.zcxfj(${item.id}, '${item.name}', ${item.change})" style="background-color:red;color:#fff;">[分解]</a>
                                </td>
                            </tr>
                            `
                ...
                var pre = pageNow - 1 > 0 ? pageNow - 1 : 0;//上一页
                var next = pageNow + 1 > pageTotal ? 0 : pageNow + 1;//下一页
                $("#showMyGiftContent2 .now_page").html(pageNow + "/" + pageTotal);
                $("#showMyGiftContent2 .now_page").attr("now_page", pageNow);
                $("#showMyGiftContent2 .btn_prev").attr("href", "javascript:Hx.zcx(" + pre + ");");
                $("#showMyGiftContent2 .btn_next").attr("href", "javascript:Hx.zcx(" + next + ");");
    ...
    zcxlq(rid, name) {
        ...
        confirm("本次领取操作不可逆，无法撤回，请确认是否操作", function () {
            var msg = '您确定领取【' + name + '】到【' + Hx.FareaName + '】下的【' + Hx.FroleName + '】吗？';
    ...
    dhlq(item) {
        ...
        confirm("本次兑换操作不可逆，无法撤回，请确认是否操作", function () {
            var msg = '您确定使用【' + Hx.dhMap[item][0] + '】积分兑换【' + Hx.dhMap[item][1] + '】到【' + Hx.FareaName + '】下的【' + Hx.FroleName + '】吗？';
            confirm_dt(msg, function () {
    ...
    request(getDhInfoApi, 'get', {}, false).then(res => {
        if (res.code == 200) {
            res.data.forEach(item => {
                $('#item_' + item).addClass('gray').text('已兑换')
            })
        }
    })
}
